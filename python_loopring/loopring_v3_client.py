...
from python_loopring.request_utils.rest_client import RestClient
from python_loopring.ethsnarks.eddsa import PureEdDSA, PoseidonEdDSA
from python_loopring.ethsnarks.field import FQ, SNARK_SCALAR_FIELD
from python_loopring.ethsnarks.poseidon import poseidon_params, poseidon
from python_loopring.sig_utils.ecdsa_utils import *
from python_loopring.sig_utils.eddsa_utils import *
...
class LoopringV3Client(RestClient):
    ...
    def sign(self, request):
        ...
        if security & Security.EDDSA_SIGN:
            ...
            print("self.eddsaKey", self.eddsaKey)
            print("LOOPRING_REST_HOST", LOOPRING_REST_HOST)
            print("signer", signer)
            print("inside2")
            signature = signer.sign(request)
            print("inside3")
            headers.update({"X-API-SIG": signature})
            print("signature X-API-SIG= ", signature)
    ...
    def mount_send_order(self, clientOrderId, base_token, quote_token, buy, price, volume, ammPoolAddress = None):
        order = self._mount_order(clientOrderId, base_token, quote_token, buy, price, volume, ammPoolAddress)
        data = {"security": Security.API_KEY}
    ...
    def _mount_order(self, clientOrderId, base_token, quote_token, buy, price, volume, ammPoolAddress):
        if buy:
            tokenSId = self.tokenIds[quote_token]
            tokenBId = self.tokenIds[base_token]
            amountS = str(int(10 ** self.tokenDecimals[tokenSId] * price * volume))
    ...
    def cancel_order(self, **kwargs):
        ...
        if "orderHash" in kwargs:
            ...
            params['clientOrderId'] = str(kwargs['clientOrderId'])
        self.add_request(
            method="DELETE",
            path="/api/v3/order",
    ...
    def mount_cancel_order(self, **kwargs):
        """"""
        data = {
            "security": Security.EDDSA_SIGN
        }
    ...
    def mount_cancel_signature(self, **kwargs):
        data = {
            "security": Security.EDDSA_SIGN
        }
